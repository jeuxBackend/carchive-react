...
function InputField({ label, type = "text", value="", setValue, fieldKey, isNumber, isReadOnly=false, isRequired=true }) {
    ...
    return (
        <motion.div
            ...
            className={`flex items-center gap-2 w-full p-3 rounded-xl font-medium 
                        ${theme === "dark" ? "bg-[#323335]" : "bg-[#f7f7f7] border border-[#e8e8e8]"}`}
        >
            {label && (
                <p className={`${theme === "dark" ? "text-[#7f8082]" : "text-[#bcbcbd]"} text-[0.7rem] sm:text-[1.2rem]`}>
                    {label}:
                </p>
            )}
            ...
            <input
                ...
                className={`outline-none border-none font-medium 
                            ...
                            ${label === "Confirm Password" ? "w-[57%] sm:flex-1"
                        : label === "Password" ? "sm:flex-1 w-[75%]" : "flex-1"}`}
            />

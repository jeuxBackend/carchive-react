    ...
    >
      {loading ? (
        <div className="h-[20vh] flex items-center justify-center">
          <motion.div className="w-7 h-7 border-2 border-[#479cff] border-t-transparent rounded-full mx-auto animate-spin" />
        </div>
      ) : (
        <div className="flex items-center gap-5 sm:flex-row flex-col">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p
                ...
                } font-medium text-[1.2rem] sm:text-[1.6rem]`}
              >
          ...
          <Link
            to="/Update-Profile"
            className="bg-[#479cff] text-white py-2 px-5 rounded-lg"
          >
            {t("Update Profile")}
          </Link>
        </div>
      ...
      <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {[
          ...
          >
            <Link
              to={item.to}
              className={`p-6 rounded-xl shadow-lg transition-all flex items-center justify-between ${
                theme === "dark"
            ...
            >
              <div className="flex items-center gap-3">
                <img
                  ...
                  alt=""
                  className={item.iconSize || "w-[2.2rem]"}
                />
                <p
                  className={`lg:text-[1.3rem] xl:text-[1.1rem] 2xl:text-[1.5rem]  ${
                    theme === "dark" ? "text-white" : "text-black"
                  }`}
                >
                  {item.label}
        ...
        >
          ...
          <button
            ...
            className={`relative w-14 h-8 rounded-full transition-colors duration-300 ${
              theme === "dark" ? "bg-[#1b1c1e]" : "bg-blue-500"
            }`}
          >
            <span
              className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform duration-300 ${
                theme === "light" ? "translate-x-6 bg-white" : ""
              }`}
            ></span>
          </button>
        </motion.div>

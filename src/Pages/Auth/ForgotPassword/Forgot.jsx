...
import { portalForgot } from "../../../API/portalServices";
import { toast } from "react-toastify";




function Forgot({ open, setOpen }) {
    if (!open) return null;
    ...
    const handleForgot = async (e) => {
        ...
        if (!email) {
            ...
            setLoading(true);
            try {
                const response = await portalForgot({ email });
                if (response?.data) {
                   toast.success("An email sent on your email address")
                   setOpen(false)
                }
        ...
        <motion.div className={`bg-black/50 backdrop-blur-lg overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%)] max-h-full poppins`}
            ...
            ${theme === "dark" ? "bg-[#1b1c1e] border-2 border-[#323335]" : "bg-white border-2 border-[#ECECEC]"}`}
                ...
                >
                    ...
                    <div onClick={handleForgot} className="bg-[#479cff] font-medium py-4 cursor-pointer rounded-2xl w-full text-center text-white mb-2">

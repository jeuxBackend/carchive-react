...
const ImageModal = ({ isOpen, onClose, imageUrl, title, theme }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`p-4 border-b ${theme === "dark" ? "border-[#323335]" : "border-gray-200"
          }`}>
            ...
            <div className="w-full space-y-6">
              ...
              {driverDetailData?.image && (
                <div className="w-full flex justify-center">
                  <div className="relative">
                    <img
                      src={getImageUrl(driverDetailData.image, 'Profile_Images/')}
                      alt="Driver Profile"
                      ...
                      onClick={() => handleImageClick(
                        getImageUrl(driverDetailData.image, 'Profile_Images/'),
              ...
              {driverDetailData?.driver && (
                <div className="w-full">
                  ...
                  {driverDetailData.driver.documents && driverDetailData.driver.documents.length > 0 && (
                    <div className="mb-4">
                      <h4 className={`${theme === "dark" ? "text-gray-300" : "text-gray-700"} text-md font-medium mb-2`}>
                        License Documents ({driverDetailData.driver.documents.length})
                      </h4>
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                        {driverDetailData.driver.documents.map((docUrl, index) => {
                          ...
                          return (
                            <div key={index} onClick={() => handleImageClick(fullDocUrl, `Driver License Document ${index + 1}`)} className="relative group">
                              <img
                                src={fullDocUrl}
                                ...
                                onError={(e) => {
                                  console.log("Document image failed to load:", fullDocUrl);
                  ...
                  {driverDetailData.driver.details && driverDetailData.driver.details.length > 0 && (
                    <div className="mb-4">
                      <h4 className={`${theme === "dark" ? "text-gray-300" : "text-gray-700"} text-md font-medium mb-2`}>
                        Additional Details ({driverDetailData.driver.details.length})
                      </h4>
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                        {driverDetailData.driver.details.map((detailUrl, index) => {
                          // Use the URL as is since it's already complete in the API response
                          const fullDetailUrl = detailUrl;
                          console.log("Detail URL:", fullDetailUrl);

                          return (
                            <div onClick={() => handleImageClick(fullDetailUrl, `Driver Detail Image ${index + 1}`)} key={index} className="relative group">
                              <img
                                src={fullDetailUrl}
                                ...
                                onError={(e) => {
                                  console.log("Detail image failed to load:", fullDetailUrl);
                                  e.target.src = 'data:image/svg+xml;base64,';
                                }}
                              />

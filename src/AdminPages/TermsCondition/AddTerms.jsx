...
function AddTerms({ open, setOpen, label, fetchSettingData, keyName }) {
  if (!open) return null;
    ...
    if (open) {
      const fetchPreviousText = async () => {
        try {
          ...
          const settingText = response?.data?.data?.[0]?.[keyName] || "";
          setText(settingText);
        } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      await updateSettings({ key: keyName, value: text });
      setOpen(false);
      fetchSettingData();
    } catch (err) {
